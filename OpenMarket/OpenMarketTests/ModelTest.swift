...
class ModelTest: XCTestCase {
    ...
    func test_ItemsOfPageReponse() throws {
        let jsonData = MockData.ItemsOfPageReponseJSON.data(using: .utf8)!
        let decodedData = try JSONDecoder().decode(ItemsOfPageReponse.self, from: jsonData)
        let items = [Item(id: 1,
                          title: "MacBook Pro",
                          price: 1690,
                          currency: "USD",
    ...
    func test_ItemEditRequest() throws {
        ...
        let jsonData = MockData.ItemEditRequestJSON.data(using: .utf8)!
        let decodedData = try JSONDecoder().decode(ItemEditRequest.self, from: jsonData)
        
        let itemEditRequest = ItemEditRequest(
            title: "MacBook Pro",
