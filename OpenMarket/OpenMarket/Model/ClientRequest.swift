...
struct ClientRequest {
    var urlRequest: URLRequest = URLRequest(url: URL(string: "https://camp-open-market-2.herokuapp.com/items/1")!)
    var header: Header?
    var body: Body?
    ...
    mutating func setProperty(by description: DescriptionAboutMenu){
        ...
        case .목록조회:
            ...
            header = Header(url: URL(string: baseURLWithString + path)!,
                            method: description.rawValue, contentsType: "application/json")
        case .상품등록:
            path = "item"
            header = Header(url: URL(string: baseURLWithString + path)!,
                            method: description.rawValue, contentsType: "multipart/form-data")
        case .상품조회:
