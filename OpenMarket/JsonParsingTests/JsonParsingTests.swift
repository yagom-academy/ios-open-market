...
class JsonParsingTests: XCTestCase {
    ...
    func test_데이터를파싱했을때_pageNo값이잘들어오는지() {
        // given
        guard let fileLocation = Bundle.main.url(forResource: "products", withExtension: "json"), let data = try? Data(contentsOf: fileLocation) else { XCTFail(); return }
        
        let expression = 1
        // when
        guard let parsedData = try? JSONDecoder().decode(Products.self, from: data) else { XCTFail(); return }
        // then
